...
class Category
{
    ...
    function __construct($id)
	{
		...
		$sql = sprintf("select CategoryID, Category, Description from " . PREFIX . "Categories Where CategoryID =%d", $this->CategoryID);
		...
		$sql = sprintf("select CategoryID, SubCategoryID, Title, Description from " . PREFIX . "SubCategory where CategoryID = %d", $this->CategoryID);
		$result = mysqli_query(\IDB::conn(),$sql) or die(trigger_error(mysqli_error(\IDB::conn()), E_USER_ERROR));
		if (mysqli_num_rows($result) > 0) {
		   	while ($row = mysqli_fetch_assoc($result)){
				#create SubCategory, and push onto stack!
				$this->SubCategories[] = new SubCategory(dbOut($row['SubCategoryID']),dbOut($row['Title']),dbOut($row['Description'])); 
		   	} #end while
		} #end if
